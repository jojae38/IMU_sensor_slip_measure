...
<br>(Issue5) - 현재 ddQ 민감도를 1로 설정하였다.(민감도를 초과 시 slip이 일어났다고 가정한다.) slip시 관측되는 ddQ 값은 약 0.25 임의로 20도 정도 로봇을 들고 돌렸을 경우이다.
<br>하지만 조이스틱으로 로봇을 조작할 경우 0.3 의 민감도를 초과하는 ddQ 값이 관측된다. 즉 평범하게 회전하는게 값 변화율이 더 크다.
<br>(Issue6) - ddQ 값의 크기로 슬립을 판단하기엔 회전과 슬립을 구분할 방법이 없다. (회전이 ddQ 값이 더 큰 경우도 존재) 그래서 슬림이라 예상하는 시점(ddQ초과)에서 dQ 샘플을 합산 시 절대값이 일정 값을 초과시 슬립이라 판단 (회전은 대부분 dQ가 +-+- 반복하는 구조라 절대값 합은 상당히 낮다.(이게 각도가 변하지 않는 이유) 반면 슬립의 경우 +++- 혹은 ---+ 그래도 일정 시간 +나 -가 유지됨)  
<br>(Issue7) - visualizer 만듬 Q(각도차), dQ(각도차 미분값), ddQ(각도차 쌍미분값), ROBOT_yaw , IMU_yaw , 슬립 시 시간 위치 표현 가능
<br>(Issue8) - 로봇의 pose값이 너무 진동하기에 평균필터를 만들어 진동을 줄임
<br>(Issue9) - subscribe 혹은 publish 명을 launch 파일에서 설정 가능하게 만듦
<br>(Issue10) - publish 내용 [상태 N-노말 S-슬립 F-전복], [IMU_yaw], [Robot_yaw], [슬립 시작시 yaw], [슬립 끝날시 yaw] 토픽명 : /pub_slip_info(변경가능)
...
(Issue5 의 대응) -> 로봇의 회전 속도를 낮춰본다 개선 여부 측정
<br> 결과적으로 불가 25에서도 느린데 5정도는 되어야 회전시 슬립이 아니라고 측정됨 근본적인 해결 방법은 아님

(Issue5 의 대응) -> 로봇을 한바퀴 여러번 돌려본 결과 Imu는 2PI에 가깝게 나오지만 Robot은 더 높게 나오니 특정 수를 곱해줘서 보정을 해주자.
...
(Issue5 의 대응) -> 뭔가 이상하다 조이스틱으로 회전시 간혈적으로 슬립이라 판단한다 그에 비해 각도 차이는 변함이 없다.
...
(Issue5 의 대응) -> 이건 예상이긴 하지만 Robot의 데이터 값과 IMU의 데이터 값의 미묘한 시간 차이 때문에 dQ 값 차이가 발생하는 것 같다.
