...
<br>(Issue6) - ddQ 값의 크기로 슬립을 판단하기엔 회전과 슬립을 구분할 방법이 없다. (회전이 ddQ 값이 더 큰 경우도 존재) 그래서 슬림이라 예상하는 시점(ddQ초과)에서 dQ 샘플을 합산 시 절대값이 일정 값을 초과시 슬립이라 판단 (회전은 대부분 dQ가 +-+- 반복하는 구조라 절대값 합은 상당히 낮다. 반면 슬립의 경우 +++- 혹은 ---+ 그래도 일정 시간 +나 -가 유지됨)  
